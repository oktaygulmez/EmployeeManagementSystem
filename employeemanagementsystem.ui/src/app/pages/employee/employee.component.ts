...
export class EmployeeComponent implements OnInit {
  ...
  applyFilter() {
    this.dataSource.data = this.employees
      .filter(
        (x) =>
          !this.selectedDepartment || x.departmentId === this.selectedDepartment
      )
      .filter(
        (x) =>
  ...
  openAddDialog() {
    ...
    updatePopup.afterClosed().subscribe((updatedUser) => {
      if (updatedUser) {
        this.employeeService.add(updatedUser).subscribe(
          (result) => {
  ...
  openEditDialog(user: UpdateEmployeeDto) {
    ...
    updatePopup.afterClosed().subscribe((updatedUser) => {
      if (updatedUser) {
        this.employeeService.update(updatedUser, user.id).subscribe(
          (result) => {
  ...
  deleteEmployee(userId: any) {
    ...
    deletePopup.afterClosed().subscribe((result) => {
      if (result) {
        this.employeeService.delete(userId).subscribe(
          (result) => {
